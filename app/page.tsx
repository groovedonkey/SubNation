...
/**
 * Sublimation Nation — Generator + Editor (plain JS)
 * - BOTH presets generate at 1536×1024 (OpenAI-supported)
 * - Editor canvas sizes remain:
 *     Mugs:     7.5" × 4.5"  @ 300 DPI
 *     Tumblers: 9.7" × 8.2"  @ 300 DPI
 * - Flow: Generate → Next → Edit (move/scale/rotate, add text, font & color) → Export PNG
 * - Expects API route: POST /api/generate-image  { prompt, size }
...
const PRINT_DPI = 300;
const SIZE_PRESETS = [
  // NOTE: both presets use the same generation size (1536x1024), but different editor canvases.
  { label: "Mugs — 1536×1024", value: "1536x1024", w: 1536, h: 1024, editorInches: { w: 7.5, h: 4.5 } },
  { label: "Tumblers — 1536×1024", value: "1536x1024", w: 1536, h: 1024, editorInches: { w: 9.7, h: 8.2 } },
];
...
function coverScale(containerW, containerH, innerW, innerH) {
...
function Studio() {
  ...
  const [step, setStep] = useState("generate"); // "generate" | "edit"

  // editor state
  const [nodes, setNodes] = useState([]); // array of { id, type: "image"|"text", ... }
  ...
  }, [nodes.map((n) => (n.type === "image" ? n.src : null)).join("|")]);
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      ...
      if (n) {
        ...
        ctx.lineWidth = 1.5;
        if (n.type === "image") {
          ctx.strokeRect(-n.naturalW / 2, -n.naturalH / 2, n.naturalW, n.naturalH);
        } else {
          const metrics = ctx.measureText(n.text || " ");
          const w = metrics.width;
          const h = n.fontSize * displayScale;
  ...
  function selectAtPoint(clientX, clientY) {
    ...
    for (let i = nodes.length - 1; i >= 0; i--) {
      ...
      if (n.type === "image") {
        ...
        if (!ctx) continue;
        ctx.font = `${n.fontSize * displayScale}px ${n.fontFamily}`;
        const w = ctx.measureText(n.text || " ").width / displayScale;
        const h = n.fontSize;
        if (lx >= -w / 2 && lx <= w / 2 && ly >= -h / 2 && ly <= h / 2) {
          setSelectedId(n.id);
          dragOffsetRef.current = { dx: lx, dy: ly };
            ...
            <section className="bg-white/80 backdrop-blur rounded-3xl shadow-xl border border-white/60 p-5 space-y-4">
              <div className="flex items-end justify-between gap-3">
                <div className="flex-1">
                  ...
                  <textarea
                    id="prompt"
                    placeholder="Dream up something quirky & cool… colors, vibe, subject, style"
              ...
              <div className="relative w-full" style={{ aspectRatio: `${aspect}` }}>
                <div className="absolute inset-0 grid place-items-center rounded-2xl border border-dashed border-gray-300 bg-white overflow-hidden">
                  ...
                  {!imageUrl ? (
                    ...
                    <img src={imageUrl} alt="Generated preview" className="max-w-full max-h-full object-contain rounded-xl" draggable={false} />
                  )}
                </div>
              </div>
            </section>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <select
                            ...
                            value={sel.fontFamily || "Inter, system-ui, sans-serif"}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <input
                            ...
                            min={8}
                            max={600}
                            value={Math.round(sel.fontSize ?? 72)}
